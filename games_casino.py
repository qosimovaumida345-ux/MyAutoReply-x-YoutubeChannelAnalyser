...
def render_apple_ui(session_data: dict, game_state: dict):
    ...
    header = (
        ...
        f"🔒 <b>Provably Fair:</b> <code>{enc_hash}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
    )

    if status == "active":
        header += f"👉 <b>{cur_row + 1}-bosqich</b> (Koeffitsient: <b>x{APPLE_MULTIPLIERS[cur_row]}</b>). Olmani tanlang:\n"
    elif status == "won":
    ...
    for r in visible_rows:
        ...
        level_mark = f"◀️ x{r_mult}" if (status == "active" and r == cur_row) else f"x{r_mult}"
...
def render_blackjack_ui(session_data: dict, game_state: dict):
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 <b>Holat:</b> {status_title}\n"
        f"{ce('MONEY')} <b>Garov:</b> <code>{bet:,} so'm</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🤖 <b>Diler (Bot) kartalari:</b>\n"
    ...
    if status == "active":
        ...
        if status == "won":
            text += f"💰 <b>Yutuq miqdori:</b> <code>+{win_uzs:,} so'm</code> hisobingizga qo'shildi!"
        elif status == "push":
            text += f"↩️ <b>Garov:</b> <code>{bet:,} so'm</code> qaytarildi."
        else:
            text += f"💸 <b>Garov:</b> <code>{bet:,} so'm</code> boy berildi."
    ...
    for s in visible_steps:
        ...
        for c in range(5):
            ...
            if k in revealed:
                ...
                lbl = "☁️" if s > cur_step else "▫️"
                cb = "kami_noop"
            row_btns.append(InlineKeyboardButton(lbl, callback_data=cb))
        
        indicator = f"◀️ x{s_mult:.2f}" if (status == "active" and s == cur_step) else f"x{s_mult:.2f}"
...
def render_crash_ui(tg_user_id: int):
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{flight_display}\n\n"
        f"📊 <b>Oxirgi koeffitsientlar:</b>\n"
        f"{recent_str}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"1️⃣ <b>1-Slot:</b> {slot1_txt}\n"
        f"2️⃣ <b>2-Slot:</b> {slot2_txt}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🏆 <b>Jonli Yutuqlar (Live):</b>\n"
        f"{lb_text}\n"
    )
    ...
    if state == "waiting":
        ...
        if not b1:
            row_bet1 = [
                InlineKeyboardButton("1️⃣ Tikish: 3k", callback_data="crash_bet_1_3000"),
                InlineKeyboardButton("1️⃣ 5k", callback_data="crash_bet_1_5000"),
                InlineKeyboardButton("1️⃣ 10k", callback_data="crash_bet_1_10000"),
            ]
        row_bet2 = []
        if not b2:
            row_bet2 = [
                InlineKeyboardButton("2️⃣ Tikish: 5k", callback_data="crash_bet_2_5000"),
                InlineKeyboardButton("2️⃣ 10k", callback_data="crash_bet_2_10000"),
                InlineKeyboardButton("2️⃣ 25k", callback_data="crash_bet_2_25000"),
            ]
